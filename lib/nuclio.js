...
const { reconcileLoop, getStatus, getInvocationUrls } = require('./nuclio-reconcile');
const { createClient } = require('./nuclio-client');
const { debounced, parseIntFallback, asString, nestedAssign, redactPaths, retryBackoff, TRANSIENT_ERR_CODES, TRANSIENT_HTTP_STATUSES } = require('./util');
...
const _TRANSIENT_ERR = new Set(TRANSIENT_ERR_CODES);
const _TRANSIENT_HTTP = new Set(TRANSIENT_HTTP_STATUSES);
const isTransient = (err) => _TRANSIENT_ERR.has(err?.code) || _TRANSIENT_HTTP.has(err?.response?.status);
...
module.exports = function(RED) {
    ...
    function NuclioServer(config) {
        ...
        this.invocationUrlPreference = ['auto', 'internal', 'external'].includes(config.invocationUrlPreference)
            ? config.invocationUrlPreference
            : 'auto';
        this.externalInvocationProtocol = config.externalInvocationProtocol === 'http' ? 'http' : 'https';
        // Service mode avoids the container/pod IPs reported by the dashboard.
        // Use the environment value when older flow files have no explicit
        // mode; dashboard-reported internal URLs remain the safe default.
        const configuredInvocationMode = config.internalInvocationMode && config.internalInvocationMode !== 'auto'
            ? config.internalInvocationMode
            : process.env.NUCLIO_INTERNAL_INVOCATION_MODE;
        this.internalInvocationMode = ['dashboard', 'service'].includes(configuredInvocationMode)
            ? configuredInvocationMode
            : 'dashboard';
        ...
        node.on("input", async function(msg, send, done) {
            ...
            try {
                ...
                for (let attempt = 0; ; attempt++) {
                    ...
                    try {
                        response = await axios.post(fnNode.invocationUrl, msg.payload, { headers, timeout: node.timeoutMs });
                        ...
                        // Transient failures (connectivity / scale-from-zero) are worth
                        // retrying; anything else is a real answer from the function.
                        // Retries are at-least-once: a dropped connection may still
                        // have delivered the request.
                        const canFailover = _TRANSIENT_ERR.has(err?.code)
                        ...
                        if (transientError && attempt < node.retries && !node.closed) {
                            const delayMs = retryBackoff(attempt + 1, node.retryDelayMs, response?.headers?.['retry-after']);
                        ...
                        if (transientError) {
                            // connectivity errors are transient (function scaling/redeploying) - warn only, no Catch nodes
    ...
    const nodeRequest = (func, description) => async (req, res) => {
        ...
        if (functionNode.configError) return res.status(400).send({ error: functionNode.configErrorReason || 'Configuration error' });
    ...
    const clientFor = (node) => {
        if (!node._client) node._client = createClient(node.server, node.project?.name || 'default');
        return node._client;
    };
    ...
    const functionStatusView = (data, view, node) => {
        ...
        if (view === 'summary') {
            return {
                ...
                invocation: {
                    mode: node?.server?.internalInvocationMode || 'dashboard',
                    preference: node?.server?.invocationUrlPreference || 'auto',
        ...
        if (view === 'spec') {
            return { metadata: data?.metadata, spec: data?.spec };
        }
        return null;
    };
    ...
    RED.httpAdmin.get(`/nuclio/api/functions`, RED.auth.needsPermission('flows.read'), nodeRequest(async (node, req, res) => {
        ...
        if (!data) return res.status(400).send({ error: `Unknown function view "${view}"` });
    ...
    RED.httpAdmin.post(`/nuclio/api/functions/deploy`, RED.auth.needsPermission('flows.write'), nodeRequest(async (node, req, res) => {
        // Redeploy function (?rebuild=true forces a full image rebuild -
        // re-fetches git/archive source behind an unchanged URL)
        await deployFunction(node, { force: true, rebuild: req.query.rebuild === 'true' });
        let r = await clientFor(node).getFunction(node.name);
    ...
    RED.httpAdmin.get(`/nuclio/api/functions/logs`, RED.auth.needsPermission('flows.read'), nodeRequest(async (node, req, res) => {
        ...
        let r = await client.getReplicas(node.name);
        const replicas = r.data?.names;

        let logs = await Promise.allSettled((replicas||[]).map(async (replica) => await client.getLogs(node.name, replica)));
