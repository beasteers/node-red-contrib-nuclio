    ...
    function NuclioInvokeNode(config) {
        ...
        node.on("input", async function(msg, send, done) {
            ...
            try {
                ...
                transientError = err?.code === 'ECONNREFUSED' || err?.code === 'ECONNABORTED' || err?.code === 'ENOTFOUND';
                if (transientError) {
                    // connectivity errors are transient (function scaling/redeploying) - warn only, no Catch nodes
                    node.warn(`Function invocation error[${err?.code}]: ${fnNode.invocationUrl} ${fnNode.fnConfigSpec?.name} ${fnNode.fnState} - ${err.message}`);
                }
