...
const createClient = (server, projectName, { signal } = {}) => {
    ...
    return {
        ...
        // Nuclio deletes functions through the collection endpoint. The
        // function identity is carried in the resource body, unlike GET/PUT
        // operations which use the name in the URL.
        deleteFunction: (name, { ignoreStateValidation = false } = {}) =>
            request('deleteFunction', {
                method: 'delete',
                url: `${address}/api/functions`,
                data: {
                    metadata: {
                        name,
                        namespace: server.namespace || 'nuclio',
                    },
                },
                headers: {
                    ...deployOpts.headers,
                    ...(ignoreStateValidation
                        ? { 'X-nuclio-delete-function-ignore-state-validation': 'true' }
                        : {}),
                },
