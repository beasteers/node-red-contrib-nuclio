...
const { diff, merge, isTransientErrorCode, isTransientHttpStatus, nodeNow } = require('./util');
...
} = require('./nuclio-function-config');

const DEPLOYMENT_ACTIONS = new Set(['reconcile', 'redeploy', 'rebuild']);
...
const removeDeletedManagedSpecPaths = (functionConfig, desiredConfig, previousConfig = functionConfig) => {
    const oldPaths = parseManagedSpecPaths(
        previousConfig.metadata?.annotations?.[MANAGED_SPEC_PATHS_ANNOTATION],
    );
    if (!oldPaths.length) return false;

    const desiredPaths = new Set(managedSpecPaths(desiredConfig.spec));
    let removed = false;
    for (const path of oldPaths) {
        if (!desiredPaths.has(path)) {
            deletePath(functionConfig, path);
            removed = true;
        }
    }
    return removed;
...
const defaultHttpTriggerDisabled = spec => spec?.[DEFAULT_HTTP_TRIGGER_FIELD] === true
    || spec?.disableDefaultHttpTrigger === true;

const removeStaleDefaultHttpTrigger = (functionConfig, desiredConfig) => {
    const desiredSpec = desiredConfig?.spec || {};
    const desiredTriggers = desiredSpec.triggers;
    const hasExplicitDefault = desiredTriggers
        && typeof desiredTriggers === 'object'
        && Object.prototype.hasOwnProperty.call(desiredTriggers, 'default-http');
    if (!defaultHttpTriggerDisabled(desiredSpec) || hasExplicitDefault) return false;

    const liveTriggers = functionConfig.spec?.triggers;
    if (!liveTriggers || typeof liveTriggers !== 'object'
        || !Object.prototype.hasOwnProperty.call(liveTriggers, 'default-http')) return false;
    delete liveTriggers['default-http'];
    return true;
};
...
async function deployFunctionConfig(node, {
    ...
    const shouldRedeploy = action !== 'reconcile';
    ...
    if (funcExists && r.status === 200) {
        ...
        if (canUpdateFunction(state)) {
            ...
            const managedFieldsRemoved = removeDeletedManagedSpecPaths(newFunc, configBody, oldFunc);
            const staleDefaultHttpRemoved = removeStaleDefaultHttpTrigger(newFunc, configBody);
            ...
            if (rebuild) {
                ...
                configChanged = existingHash !== desiredHash
                    || managedFieldsRemoved
                    || staleDefaultHttpRemoved;
            } else {
                diffResult = diff(newFunc, { apiVersion: newFunc.apiVersion, kind: newFunc.kind, ...oldFunc });
                configChanged = !_.isEmpty(diffResult) || managedFieldsRemoved || staleDefaultHttpRemoved;
            }

            // No differences!
            if (!configChanged) {
                // Function isn't ready, just patch the desired state to ready
                if ((state !== 'ready') || shouldRedeploy) {
...
// Deploy the function config. Returns true if the request was accepted, false
// if it failed, undefined if skipped (already deploying).
...
// `action: rebuild` forces a full image rebuild (re-fetches git/archive source)
// even when the config is unchanged. `action: redeploy` reuses the existing
// image and asks Nuclio to make the function ready.
const runDeployFunction = async (node, _options = {}) => {
    const { action = 'reconcile', ...opts } = _options;
    if (!DEPLOYMENT_ACTIONS.has(action)) {
        throw new Error(`Invalid deployment action "${action}"`);
    }
    ...
    if ((node.redeploying && action === 'reconcile') || !node.fnConfigSpec) return;
    // An explicit action is an intentional "try again" - un-give-up the
    // self-healer, while ordinary reconciliation remains passive.
    if (action !== 'reconcile') node.selfHealAttempts = 0;
    node.redeploying = true;
    node.redeployDeadline = nodeNow(node) + node.redeployDeadlineMs;
    ...
    try {
        ...
        const { changed } = await deployFunctionConfig(node, { config: configBody, action });
        if (changed || action !== 'reconcile') invalidateStatus(node);
        ...
        if (isConnErr) {
            ...
            const responseDetail = typeof responseData === 'string'
                ? responseData
                : responseData?.error?.message || responseData?.error || responseData?.message || responseData?.reason;
            const detailText = typeof responseDetail === 'string'
                ? responseDetail.replace(/[\r\n\t]+/g, ' ').trim().slice(0, 300)
                : '';
            const detail = r?.status
                ? `dashboard returned HTTP ${r.status}${detailText ? `: ${detailText}` : ''}`
                : 'deployment request failed';
            node.error(`Error while deploying function ${node.name}: ${detail}`);
        }
        node.status({ fill: "red", shape: "ring", text: r?.status ? `Error ${r.status}` : 'Deployment failed' });
        return false;
    }
};
...
const deployFunction = (node, options = {}) => {
    ...
    const tracked = promise.then(result => {
        const outcome = result === false ? 'deferred' : result === undefined ? 'skipped' : 'accepted';
        getMetrics(node.server).recordDeployment(node, outcome, Date.now() - startedAt);
        return result;
    }, err => {
        getMetrics(node.server).recordDeployment(node, 'error', Date.now() - startedAt);
        throw err;
    }).finally(() => {
...
const undeployFunction = async (node) => {
    const name = node.fnConfigSpec?.name || node.name;
    const client = getClient(node);
    let current;

    try {
        current = await client.getFunction(name);
    } catch (err) {
        if (err.response?.status === 404) return { accepted: true, deleted: false, name };
        throw err;
    }

    const annotations = current.data?.metadata?.annotations || {};
    if (annotations['nuclio.io/node-red'] !== 'true') {
        throw new Error(`Refusing to undeploy unowned Nuclio function ${name}`);
    }

    try {
        await client.deleteFunction(name, { ignoreStateValidation: true });
        return { accepted: true, deleted: true, name };
    } catch (err) {
        // Treat a concurrent delete as success so the command is idempotent.
        if (err.response?.status === 404) return { accepted: true, deleted: false, name };
        throw err;
    }
};
