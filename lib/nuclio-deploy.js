...
const removeDeletedManagedSpecPaths = (functionConfig, desiredConfig, previousConfig = functionConfig) => {
    ...
    for (const path of oldPaths) {
        if (!desiredPaths.has(path)) deletePath(functionConfig, path);
    }
};
...
async function deployFunctionConfig(node, {
    ...
    if (funcExists && r.status === 200) {
        ...
        if (canUpdateFunction(state)) {
            ...
            removeDeletedManagedSpecPaths(newFunc, configBody, oldFunc);
...
const runDeployFunction = async (node, _options = {}) => {
    ...
    try {
        ...
        if (isConnErr) {
            ...
            const detail = r?.status ? `dashboard returned HTTP ${r.status}` : 'deployment request failed';
