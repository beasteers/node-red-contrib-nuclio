...
const replicaText = fnNode => {
    ...
    const spec = fnNode?.statusSnapshot?.spec || {};
    const ready = firstNumeric(status.readyReplicas, status.availableReplicas, status.activeReplicas);
    const desired = firstNumeric(status.desiredReplicas, status.replicas, spec.replicas);
