...
const getReplicaStatus = async node => {
    try {
        const response = await getClient(node).getReplicas(node.name);
        const names = response.data?.names;
        return Array.isArray(names)
            ? { activeReplicas: names.length }
            : { replicaStatusError: 'invalid-response' };
    } catch (err) {
        // Replica data is supplemental. A dashboard/platform that does not
        // expose the endpoint must not make an otherwise healthy function
        // appear unavailable. Other failures remain visible in the status
        // summary so missing capacity is not mistaken for zero replicas.
        if (err.response?.status === 404) return {};
        return {
            replicaStatusError: err.response?.status
