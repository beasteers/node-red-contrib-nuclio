...
const { getTriggerSummaries, hasHttpTrigger } = require('./nuclio-invocation-urls');
...
const summarizeFunction = (func, replicaStatus = {}) => {
    ...
    return {
        ...
        // Keep trigger semantics alongside the reduced status snapshot. The
        // full function spec is intentionally omitted from editor polling,
        // but a cached NATS-only function must not be mistaken for HTTP.
        httpTrigger: typeof func?.httpTrigger === 'boolean'
            ? func.httpTrigger
            : hasHttpTrigger(func),
