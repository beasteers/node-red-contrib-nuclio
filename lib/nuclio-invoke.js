...
const TRANSIENT_ERR = new Set(TRANSIENT_ERR_CODES);
const TRANSIENT_HTTP = new Set(TRANSIENT_HTTP_STATUSES);
const isTransient = (err) => TRANSIENT_ERR.has(err?.code) || TRANSIENT_HTTP.has(err?.response?.status);
    ...
    try {
        ...
        for (let attempt = 0; ; attempt++) {
            ...
            try {
                ...
                const canFailover = TRANSIENT_ERR.has(err?.code)
