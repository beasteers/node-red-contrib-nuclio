...
const { validateFunctionName, validateSourcePath } = require('../resources/nuclio-validation');
...
// Nuclio returns several sensitive trigger fields as ordinary object keys.
// Redact them defensively before a function document is exposed through the
// Node-RED admin API, including fields authored directly in YAML or returned
// from an existing function rather than Node-RED credential entries.
const NUCLIO_SENSITIVE_KEYS = new Set([
    'password',
    'passphrase',
    'token',
    'secret',
...
const normalizeSensitiveKey = key => `${key}`.toLowerCase().replace(/[-_]/g, '');

const redactNuclioSensitiveValues = (value, replacement = '[redacted]') => {
    if (Array.isArray(value)) return value.map(item => redactNuclioSensitiveValues(item, replacement));
    if (!value || typeof value !== 'object') return value;
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [
        key,
        NUCLIO_SENSITIVE_KEYS.has(normalizeSensitiveKey(key))
            ? replacement
            : redactNuclioSensitiveValues(item, replacement),
    ]));
};
...
// default. Environment values are used only when the field type is `env`.
...
// Production nodes use wall-clock time. Tests may provide a deterministic
// clock on the node without changing runtime behavior.
const nodeNow = node => typeof node?.clock?.now === 'function' ? node.clock.now() : Date.now();
