...
// How long a redeploy may run before the reconcile loop treats it as failed (so a
// deploy that never restores health can't pin a stale "Redeploying..." forever).
const REDEPLOY_DEADLINE_MS = parseIntFallback(process.env.NUCLIO_REDEPLOY_DEADLINE_MS, 120000);
...
const deployFunction = async (node, options={}) => {
    ...
    node.redeployDeadline = Date.now() + REDEPLOY_DEADLINE_MS;
