...
async function deployFunctionConfig(node, address, {
    ...
    if (r && r.status === 200) {
        ...
        if (!(BUILDING.includes(state) || WAITING.includes(state))) {
            ...
            // have them yet - fall back to the deep diff for those.
            let configChanged;
            let diffResult;
            if (existingHash) {
            ...
            if (!configChanged) {
                ...
                // (this PUT migrates them to hash-based detection).
                const specDiff = diffResult?.spec || {};
                const buildUnchanged = existingHash
                ...
                node.log(`Updating function ${name}...`);
