...
async function buildFunctionConfig({
    name,
    code,
    config,
    runtime,
    ...
    return {
        ...
        metadata: {
            ...
            annotations: {
                ...
                // 'nuclio.io/force-redeploy': force ? 'true' : 'false',
                // 'nuclio.io/server-address': address,
            },
        },
        spec: {
            runtime: runtime,
...
async function deployFunctionConfig(address, {
    config: configBody,
    force,
}) {
    ...
    if (!foundProject) {
        console.log(`NUCLIO Creating project ${projectName}...`);
    ...
    if (r && r.status === 200) {  // REVIEW
        ...
        if (!(BUILDING.includes(state) || WAITING.includes(state))) {
            ...
            // console.log("NUCLIO Function diff:", diffResult);
            // console.log("oldFunc", oldFunc.spec.build.triggers);
            // console.log("newFunc", configBody.spec.build.triggers);
            // console.log("invDiffResult", invDiffResult);
...
const deployFunction = async (node, options={}) => {
    ...
    try {
        ...
        const configBody = await buildFunctionConfig(options);
        await deployFunctionConfig(address, { config: configBody, force });
    } catch (err) {
        const r = err.response;
        if (r && (r.status === 404 || r.status === 502 || r.status === 503)) {
            // console.warn(err.status, err.code, r.status, r.code)
            // Function not found or server error, back off and retry
