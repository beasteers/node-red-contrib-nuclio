...
const { diff, merge, hashConfig } = require('./util');
...
const deployFunction = async (node, options={}) => {
    if (options.rebuild) options.force = true;
    ...
    try {
        ...
        const { force, rebuild, address } = options;
        ...
        const isConnErr = ['ECONNREFUSED','ECONNRESET','ECONNABORTED','ETIMEDOUT','ENOTFOUND','EAI_AGAIN'].includes(err?.code);
        if (isConnErr) {
