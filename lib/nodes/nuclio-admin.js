...
const { getInvocationUrlOptions, getTriggerSummaries } = require('../nuclio-invocation-urls');
...
const { redactPaths, redactNuclioSensitiveValues } = require('../util');
...
module.exports = function(RED) {
    ...
    const nodeRequest = (func, description) => async (req, res) => {
        ...
        try {
            ...
            if (isCircuitOpenError(err)) {
                functionNode.status({ fill: 'red', shape: 'ring', text: 'Nuclio unavailable' });
                const retryAfterSeconds = Math.max(1, Math.ceil((err.retryAfterMs || 0) / 1000));
                res.setHeader('Retry-After', `${retryAfterSeconds}`);
                return res.status(503).send({ error: err.message, retryAfterSeconds });
            }
    ...
    const redactFunctionData = (node, data) => redactNuclioSensitiveValues(
        redactPaths(data, node.secretVarPaths),
    );
    ...
    const functionStatusView = (data, view, node) => {
        ...
        if (view === 'summary') {
            ...
            const invocation = getInvocationUrlOptions(data, node);
            return {
                ...summary,
                invocation,
            };
    ...
    RED.httpAdmin.get('/nuclio/api/functions', RED.auth.needsPermission('flows.read'), nodeRequest(async (node, req, res) => {
        // Summary is safe and small enough for editor polling. Full/spec views
        // are explicit because Nuclio function documents may contain trigger
        // credentials and other control-plane secrets.
    ...
    RED.httpAdmin.get('/nuclio/api/functions/triggers', RED.auth.needsPermission('flows.read'), nodeRequest(async (node, req, res) => {
        // Trigger capabilities come from Node-RED's resolved configuration, so
        // the editor can explain lazy or not-yet-deployed functions without a
        // dashboard status request.
        return res.status(200).send({
            triggerSummaries: getTriggerSummaries(node.fnConfigSpec?.config || {}),
        });
    }, 'getting function trigger configuration'));
    ...
    RED.httpAdmin.post('/nuclio/api/functions/deploy', RED.auth.needsPermission('flows.write'), nodeRequest(async (node, req, res) => {
        ...
        const deployOptions = {
            action: req.query.rebuild === 'true' ? 'rebuild' : 'redeploy',
            wakeReconcile: false,
        };
        const accepted = await (node.activateDeployment
    ...
    RED.httpAdmin.get('/nuclio/api/metrics', RED.auth.needsPermission('flows.read'), nodeRequest(async (node, req, res) => {
        res.setHeader('Content-Type', 'text/plain; version=0.0.4; charset=utf-8');
        return res.status(200).send(getMetrics(node.server).toPrometheus());
    }, 'getting metrics'));
