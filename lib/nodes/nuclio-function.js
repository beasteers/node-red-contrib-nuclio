...
const { asString, boolSetting, nestedAssign, normalizeConfigPath, numSetting, validateFunctionName, validateSourcePath } = require('../util');
...
module.exports = function(RED) {
    ...
    const getDeploymentVariables = (node, config) => {
        ...
        try {
            if (node.credentials?.deploymentVariables) {
    ...
    function NuclioFunctionConfig(config) {
        ...
        if (!node.configError) {
            try {
                ...
                const resolvedConfig = getConfig(node, config, secretVars);
