...
const applyExecutionConfig = (RED, node, configData, config) => {
    const triggerName = optionalSetting(RED, node, config, 'executionTriggerName') || 'http';
    ...
    const hasSettings = [mode, batchMode, batchSize, batchTimeout, workers, eventTimeout]
    ...
    const trigger = { ...(triggers[triggerName] || {}), kind: (triggers[triggerName] || {}).kind || 'http' };
