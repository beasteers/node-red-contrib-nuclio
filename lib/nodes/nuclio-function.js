...
const SOURCE_TYPES = new Set(['sourceCode', 'image', 'git', 'archive']);
...
const inferSourceType = (configData) => {
    ...
    return SOURCE_TYPES.has(codeEntryType) ? codeEntryType : 'sourceCode';
};
...
const applySourceConfig = (configData, config, credentials = {}) => {
    ...
    if (sourceType === 'sourceCode' && !hasExplicitSourceType) return sourceType;
