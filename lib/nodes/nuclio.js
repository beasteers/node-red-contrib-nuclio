...
const COMMANDS = new Set(['deploy', 'redeploy', 'rebuild', 'status']);
    ...
    function NuclioInvokeNode(config) {
        ...
        const waitForReady = (fnNode) => new Promise((resolve, reject) => {
            ...
            const check = () => {
                ...
                if (fnNode.fnState === 'ready' && fnNode.invocationUrl && !fnNode.redeploying) return finish(resolve);
        ...
        const commandStatus = (fnNode) => ({
            ...
            ready: fnNode.fnState === 'ready' && Boolean(fnNode.invocationUrl) && !fnNode.redeploying,
        ...
        const queueCommand = (command, msg, send, done) => {
            const next = node.commandPromise.then(
                () => executeCommand(command, msg, send, done),
                () => executeCommand(command, msg, send, done),
            );
            node.commandPromise = next.catch(() => {});
            return next;
        };
        ...
        node.on("input", async function(msg, send, done) {
            ...
            if (command) return queueCommand(command, msg, send, done);
            ...
            if (fnNode.deploymentMode === 'lazy' && !fnNode.lazyActivated) return drop(ring("Waiting for deploy"));
