...
module.exports = function(RED) {
    ...
    function NuclioInvokeNode(config) {
        ...
        node.pendingInvocations = new Set();
        if (node.function) {
        ...
        node.on("close", function(_removed, done) {
            ...
            if (typeof done === 'function') {
                Promise.allSettled(node.pendingInvocations).then(() => done());
            }
        });
        try {
        ...
        node.on("input", async function(msg, send, done) {
            ...
            const invocation = invokeWithRetry({ node, fnNode, msg, getHeaders: () => buildHeaders(msg) });
            node.pendingInvocations.add(invocation);
            const result = await invocation.finally(() => node.pendingInvocations.delete(invocation));
