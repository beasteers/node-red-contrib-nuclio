...
const resolveAuthValue = (RED, node, config, field) => {
    const type = config[`${field}Type`]
        || (node.credentials?.[field] || node.credentials?.[`has_${field}`] ? 'cred' : 'str');
    const value = config[`${field}Value`] ?? config[field] ?? '';
    if (type === 'cred') {
        const credential = node.credentials?.[field];
        return credential === '__PWRD__' ? '' : (credential || '');
    }
    return type === 'env'
        ? RED.util.evaluateNodeProperty(value, 'env', node)
        : value;
};
...
module.exports = function(RED) {
    function NuclioServer(config) {
        ...
        const authUsername = config.authUsername
            ...
            : '';
        const authPassword = resolveAuthValue(RED, this, config, 'authPassword');
        const authToken = resolveAuthValue(RED, this, config, 'authToken');
        let requestHeaders = [];
        try {
