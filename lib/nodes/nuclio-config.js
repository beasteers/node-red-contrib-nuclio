...
const resolveAuthValue = (RED, node, config, field) => {
    const type = config[`${field}Type`] || (node.credentials?.[field] ? 'cred' : 'str');
    const value = type === 'cred'
        ? node.credentials?.[field]
        : type === 'env'
            ? RED.util.evaluateNodeProperty(config[field], 'env', node)
            : config[field];
    return value === '__PWRD__' ? '' : value;
};
